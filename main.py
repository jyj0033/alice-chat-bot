...
class GroupChatBot:
    ...
    def _init_memory(self) -> None:
        ...
        # 用户画像：从情景记忆提炼爱丽丝对每个群友的第一人称印象（semantic 记忆）
    ...
    _PROFILE_DOSSIER_MARKERS = ("该用户", "此人", "该群友")
    _PROFILE_FIRST_PERSON_MARKERS = ("我", "印象里")
    ...
    @classmethod
    def _profile_distill_system_prompt(cls, bot_name: str = "爱丽丝") -> str:
        """爱丽丝第一人称印象，不是旁观者档案。"""
        name = (bot_name or "爱丽丝").strip() or "爱丽丝"
        return (
            f"你在帮{name}整理她对群友的印象。用{name}的第一人称来写，"
            "像她自己记得这个人，而不是旁观者在做人物档案。\n"
            "根据【自我描述】和【日常发言】，写出她对这个人长期稳定的印象"
            "（职业、常玩的游戏、作息、习惯、性格、常聊话题，以及相处时反复出现的感觉）。\n"
            "- 【自我描述】是对方直接说过的关于自己的话，较可信\n"
            "- 【日常发言】是对方的日常聊天，只从反复出现的模式里归纳\n"
            "- 两类素材都是从群聊里摘出来的孤立单句，很多是在回别人的话，"
            "看不到上下文。读不懂或像是在接别人话茬的，直接跳过，不要硬解释。\n"
            "- 【对话上下文】只用于理解【目标发言】的指代和语气；上下文里的其他人"
            "说过的兴趣、经历、身份和观点，绝对不能归到这个人身上。\n"
            "- 【已有印象】是她之前根据更早聊天留下的记忆（本次资料里可能不再提到）\n"
            "\n给了【已有印象】时，要在它的基础上更新，而不是重写：\n"
            "A. 保留仍然成立的稳定印象（职业、所在地、长期爱好），"
            "即使这次资料里没再提到也要留着——人不会因为最近没说就不是那样的人。\n"
            "B. 新资料和已有印象冲突时以新资料为准，直接改掉旧结论。\n"
            "C. 新资料里发现的新的稳定特征，补充进去。\n"
            "D. 已有印象里明显是一次性事件、或事后看判断错的，删掉。\n"
            "E. 如果已有印象是第三人称档案口吻（该用户/此人/TA常…），"
            "改写成第一人称，事实保留。\n"
            "\n判断标准：\n"
            ...
            "「发言活跃」。只写能把这个人和别人区分开的信息。\n"
            "5. 不要猜年龄、收入、家境；性别没明说就用对方名字，不要写「他/她」「TA」。\n"
            "6. 用第一人称：以「我」来写，可以说「我记得」「印象里」「我觉得」。"
            "点名用对方名字。不要用「该用户」「此人」「TA」。"
            "不复述原话，不解释推理过程。\n"
            "7. 相处感觉只能写聊天里反复出现的，没有依据就不要写好感或讨厌。\n"
            "8. 只能写这份资料或已有印象里出现过的内容。没出现过的职业、地名、游戏名，"
            "一个字都不能出现——绝对不要把别人的情况套到这个人身上。\n"
            "\n粒度要求：写具体的东西——做什么工作或在哪读书、常玩什么、什么作息或"
            "习惯、说话有什么特点，每一项都必须有出处；"
            "不要写「是个游戏爱好者」「喜欢和大家聊天」这种笼统评价。\n"
            "\n输出一段完整的第一人称印象（不是修改说明、不要分点、不要加"
            "「更新后画像：」「我的印象：」这类开头），必须控制在120字以内——"
            "这是硬性要求，累积的内容变多时要做取舍，只保留最能代表这个人的信息。\n"
            "只要能找到一条具体且反复出现的信息，就要写出来。"
            "只有当资料和已有印象里都找不到任何具体信息时，才输出「无法提炼」四个字。"
        )

    @classmethod
    def _profile_quality_warnings(cls, summary: str) -> list:
        """标出画像里的可疑表述（推测、档案口吻、空话），供人工复核。"""
        text = summary or ""
        warnings = []
        ...
        dossier = [m for m in cls._PROFILE_DOSSIER_MARKERS if m in text]
        if dossier:
            warnings.append("档案口吻：" + "、".join(dossier))
        if text and not any(m in text for m in cls._PROFILE_FIRST_PERSON_MARKERS):
            warnings.append("不是第一人称")
    ...
    async def _distill_user_profiles(self, force: bool = False) -> dict:
        ...
        每个发送者的画像由两部分信号合成，写成爱丽丝的第一人称印象：
        - 【自我描述】对方直接说过的关于自己的话（较可信）
        - 【日常发言】对方的日常聊天，从中推断兴趣/性格/习惯等稳定倾向
        ...
        try:
            ...
            for scope_key, entry in by_scope.items():
                ...
                bot_name = getattr(getattr(self, "personality", None), "name", "") or "爱丽丝"
                req.add_system(self._profile_distill_system_prompt(bot_name))
